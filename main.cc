...
void PhysicsLoop(mj::Simulate& sim, MuJoCoBridge& bridge) {
...

    {
      ...
      if (m) {
		  ...
		  if (sim.run) {
			  ...
			  mj_forward(m, d); // make sure state info propagates to sensor fields
...
// TODO Next
// get the simpler mujoco stuff running as well; different option
// put the model files in this directory
// look for other xml model files? x500 could be urdf'd
// would then need to set up the other px4 configs
// change sitl_run.sh to work correctly
...
void PhysicsThread(mj::Simulate* sim, MuJoCoBridge* bridge, const char* filename) {
  ...
  std::cout << "MUJOCO: TIMESTEP HARDCODED TO 0.004" << std::endl;
  std::cout << "MUJOCO: THIS IS BAD AND A HACK." << std::endl;

  //std::this_thread::sleep_for(std::chrono::seconds(10));
  bridge->initialize();

  PhysicsLoop(*sim, *bridge);
  std::cout << "MUJOCO: out of physics sim loop" << std::endl;
...
int main(int argc, char** argv) {
  ...
  std::thread physicsthreadhandle(&PhysicsThread, sim.get(), &bridge, filename);

  // start simulation UI loop (blocking call)
  sim->RenderLoop();
  physicsthreadhandle.join();

  std::cout << "MUJOCO: out of render loop" << std::endl;
